...
def get_dataframe_programacao_junho_bruta():

    df = pd.read_csv("programacao_junho_bruta.csv")
...
# LEFT JOIN
def fill_hours_df_prog_junho_bruta(df_left, df_right):
    ...
    df_left['HORA INICIO MOT. 1'] = df_left['ENTRADA'].fillna(df_left['HORA INICIO MOT. 1'])
...
def main():
    ...
    df_prog_junho = get_dataframe_programacao_junho_bruta()
    df_folha_completa = get_dataframe_folha_completa_limpa()

    df_prog_junho = clean_data(df_prog_junho)
    df_folha_completa = clean_data(df_folha_completa)

    df_novo = fill_hours_df_prog_junho_bruta(df_prog_junho, df_folha_completa)

    df_novo.to_csv("start_and_end_hour_prog_junho.csv", index=False, columns=["HORA INICIO MOT. 1", "HORA FIM MOT. 1"])
